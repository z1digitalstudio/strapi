...
describe('CleanData utils', () => {
  ...
  describe('sortContentType', () => {
    ...
    it('should return sorted single types array', () => {
      const { sortedSingleTypes } = expectedData;
      const {
        rawData: { singleTypesToSort },
      } = rawData;

      const actual = sortContentType(singleTypesToSort);
      expect(actual.sort()).toEqual(sortedSingleTypes.sort());
    });
    it.only('should return an empty array if no content types', () => {
